...
route.get("/getDateLists/:catagory", async (req, res) => {
  try {
    ...
    var item;
    if (catagory == "primePlusVip") {
      item = await db.execute(
        " SELECT date as `date` FROM `match` where `type`=? and (`catagory`=? or `catagory`=?) GROUP BY DATE ORDER BY DATE DESC; ",
        [type, "eliteVip", "correctScoreVIP"]
      );
      console.log(catagory);
    } else if (catagory == "premiumVip") {
      item = await db.execute(
        "SELECT date as `date` FROM `match` where `type`=? and (`catagory`=? or `catagory`=?)  GROUP BY DATE ORDER BY DATE DESC; ",
        [type, , "htFtVip", "overUnderVip"]
      );
    } else {
      item = await db.execute(
  ...
  try {
    ...
    if (catagory == "primePlusVip") {
      match = await db.execute(
        " SELECT * FROM `match` where `date`=? and `type`=? and (`catagory`=? or `catagory`=?) ",
        [date, "vip", "eliteVip", "correctScoreVIP"]
      );
    } else if (catagory == "premiumVip") {
      match = await db.execute(
        " SELECT * FROM `match` where `date`=? and `type`=? and (`catagory`=? or `catagory`=?) ",
        [date, "vip", "htFtVip", "overUnderVip"]
      );
    } else {
      match = await db.execute(
